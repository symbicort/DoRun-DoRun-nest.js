...
export class MissionRepository {
    ...
    async findByMissionIdIn(missionIds: string[]): Promise<MissionEntity[]> {
        return this.missionRepository.find({ where: { missionId: In(missionIds) } });
    }

    async findByUserIdAndMissionId_Course(user: User, course: string): Promise<UserMissionEntity[]> {
    ...
    async findByUserIdAndCompleteAndLearn(user: User, complete: boolean, learn: boolean): Promise<UserMissionEntity[]> {
        return this.userMissionRepository.find({
            where: {
                userId: user.userId,
                complete: complete,
    ...
    async findByUserIdAndLearnAndMissionId_Course(user: User, learn: boolean, course: string): Promise<UserMissionEntity[]> {
        return this.userMissionRepository.find({
            where: {
                userId: user.userId,
                learn: learn,
    ...
    async findByUserIdAndMissionIdAndLearn(user: User, mission: MissionEntity, learn: boolean): Promise<UserMissionEntity | undefined> {
    ...
    async findByUserIdAndMissionId_MissionIdIn(user: User, missions: string[]): Promise<UserMissionEntity[]> {
        return this.userMissionRepository.find({
            where: {
                userId: user.userId,
                missionId: {
    ...
    async findByMissionId_MissionIdIn(missionIds: string[]): Promise<UserMissionEntity[]> {
        return this.userMissionRepository.find({
            where: {
                missionId: {
                    id: In(missionIds),
